...
void PatriciaTrie::printAllPaths(PatriciaNode *root, vector<std::string> elements)
{
    if (!root)
        return;

    elements.push_back(root->stringData);
    ...
    for (int i = 0;
        ...
        printAllPaths(root->childList->at(i), elements);
}
