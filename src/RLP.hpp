...
namespace dev
{
    ...
    static const std::byte c_rlpMaxLengthvector_ref<std::byte> = 8;
    static const std::byte c_rlpDataImmLenStart = 0x80;
    static const std::byte c_rlpListStart = 0xc0;

    static const std::byte c_rlpDataImmLenCount = c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthvector_ref<std::byte>;
    static const std::byte c_rlpDataIndLenZero = c_rlpDataImmLenStart + c_rlpDataImmLenCount - 1;
    static const std::byte c_rlpListImmLenCount = 256 - c_rlpListStart - c_rlpMaxLengthvector_ref<std::byte>;
    static const std::byte c_rlpListIndLenZero = c_rlpListStart + c_rlpListImmLenCount - 1;
    ...
    public:
        ...
        /// Construct a node of value given in the vector_ref<std::byte>.
        explicit RLP(vector_ref<std::byte const> _d, Strictness _s = VeryStrict);

        /// Construct a node of value given in the vector_ref<std::byte>.
        explicit RLP(vector_ref<std::byte> const &_d, Strictness _s = VeryStrict) : RLP(&_d, _s) {}

        /// Construct a node to read RLP data in the vector_ref<std::byte> given.
        RLP(std::byte const *_b, unsigned _s, Strictness _st = VeryStrict) : RLP(vector_ref<std::byte const>(_b, _s), _st) {}

        /// Construct a node to read RLP data in the string.
        explicit RLP(std::string const &_s, Strictness _st = VeryStrict) : RLP(vector_ref<std::byte const>((std::byte const *)_s.data(), _s.size()), _st) {}

        /// The bare data of the RLP.
        vector_ref<std::byte const> data() const { return m_data; }
        ...
        /// @returns the number of vector_ref<std::byte> in the data, or zero if it isn't data.
        ...
        private:
            ...
            vector_ref<std::byte const> m_currentItem;
        };
        ...
        explicit operator vector_ref<std::byte>() const { return tovector_ref<std::byte>(); }
        ...
        /// Converts to bytearray. @returns the empty std::byte array if not a string.
        vector_ref<std::byte> tovector_ref<std::byte>(int _flags = LaissezFaire) const
        {
            if (!isData())
            {
                if (_flags & ThrowOnFail)
                    ...
                    return vector_ref<std::byte>();
            }
            return vector_ref<std::byte>(payload().data(), payload().data() + length());
        }
        /// Converts to bytearray. @returns the empty std::byte array if not a string.
        vector_ref<std::byte const> tovector_ref<std::byte const>(int _flags = LaissezFaire) const
        {
            if (!isData())
            {
                if (_flags & ThrowOnFail)
                    ...
                    return vector_ref<std::byte const>();
            }
        ...
        /// Converts to int of type given; if isData(), decodes as big-endian vector_ref<std::byte>tream. @returns 0 if not an int or data.
        ...
        vector_ref<std::byte const> payload() const
        {
            auto l = length();
            if (l > m_data.size())
    ...
    private:
        ...
        explicit RLP(vector_ref<std::byte> const &&) {}

        /// Throws if is non-canonical data (i.e. single std::byte done in two vector_ref<std::byte> that could be done in one).
        void requireGood() const;

        /// Single-std::byte data payload.
        bool isSingleByte() const { return !isNull() && m_data[0] < c_rlpDataImmLenStart; }

        /// @returns the amount of vector_ref<std::byte> used to encode the length of the data. Valid for all types.
        ...
        /// @returns the size in vector_ref<std::byte> of the payload, as given by the RLP as opposed to as inferred from m_data.
        size_t length() const;

        /// @returns the number of vector_ref<std::byte> into the data that the payload starts.
        ...
        static size_t sizeAsEncoded(vector_ref<std::byte const> _data) { return RLP(_data, ThrowOnFail | FailIfTooSmall).actualSize(); }

        /// Our std::byte data.
        vector_ref<std::byte const> m_data;

        /// The list-indexing cache.
        // Index of the last item accessed with operator[]
        mutable size_t m_lastIndex = (size_t)-1;
        // Offset of the next std::byte after last std::byte of m_lastItem
        mutable size_t m_lastEnd = 0;
        // Data of the last item accessed with operator[]
        mutable vector_ref<std::byte const> m_lastItem;
    };
    ...
    template <>
    struct Converter<vector_ref<std::byte>>
    {
        static vector_ref<std::byte> convert(RLP const &_r, int _flags) { return _r.tovector_ref<std::byte>(_flags); }
    };
    template <>
    ...
    /**
     * @brief Class for writing to an RLP vector_ref<std::byte>tream.
     */
    class RLPStream
    {
    public:
        ...
        /// Append given datum to the std::byte stream.
        ...
        RLPStream &append(vector_ref<std::byte const> _s, bool _compact = false);
        RLPStream &append(vector_ref<std::byte> const &_s) { return append(vector_ref<std::byte const>(&_s)); }
        RLPStream &append(std::string const &_s) { return append(vector_ref<std::byte const>(_s)); }
        RLPStream &append(char const *_s) { return append(std::string(_s)); }
        template <unsigned N>
        RLPStream &append(FixedHash<N> _s, bool _compact = false, bool _allOrNothing = false) { return _allOrNothing && !_s ? append(vector_ref<std::byte const>()) : append(_s.ref(), _compact); }
        ...
        RLPStream &appendList(vector_ref<std::byte const> _rlp);
        RLPStream &appendList(vector_ref<std::byte> const &_rlp) { return appendList(&_rlp); }
        RLPStream &appendList(RLPStream const &_s) { return appendList(&_s.out()); }

        /// Appends raw (pre-serialised) RLP data. Use with caution.
        RLPStream &appendRaw(vector_ref<std::byte const> _rlp, size_t _itemCount = 1);
        RLPStream &appendRaw(vector_ref<std::byte> const &_rlp, size_t _itemCount = 1) { return appendRaw(&_rlp, _itemCount); }
        ...
        /// Read the std::byte stream.
        vector_ref<std::byte> const &out() const
        {
        ...
        /// Invalidate the object and steal the output std::byte stream.
        vector_ref<std::byte> &&invalidate()
        {
            if (!m_listStack.empty())
                BOOST_THROW_EXCEPTION(RLPException() << errinfo_comment("listStack is not empty"));
        ...
        /// Swap the contents of the output stream out for some other std::byte array.
        void swapOut(vector_ref<std::byte> &_dest)
        {
            if (!m_listStack.empty())
                BOOST_THROW_EXCEPTION(RLPException() << errinfo_comment("listStack is not empty"));
            swap(m_out, _dest);
    ...
    private:
        ...
        /// Push the node-type std::byte (using @a _base) along with the item count @a _count.
        /// @arg _count is number of characters for strings, data-vector_ref<std::byte> for ints, or items for lists.
        void pushCount(size_t _count, std::byte _offset);

        /// Push an integer as a raw big-endian std::byte-stream.
        template <class _T>
        void pushInt(_T _i, size_t _br)
        {
            m_out.resize(m_out.size() + _br);
            std::byte *b = &m_out.back();
            for (; _i; _i >>= 8)
                *(b--) = (std::byte)(_i & 0xff);
        }

        /// Our output std::byte stream.
        vector_ref<std::byte> m_out;
    ...
    /// Export a single item in RLP format, returning a std::byte array.
    template <class _T>
    vector_ref<std::byte> rlp(_T _t) { return (RLPStream() << _t).out(); }

    /// Export a list of items in RLP format, returning a std::byte array.
    inline vector_ref<std::byte> rlpList() { return RLPStream(0).out(); }
    template <class... _Ts>
    vector_ref<std::byte> rlpList(_Ts... _ts)
    {
    ...
    extern vector_ref<std::byte> RLPNull;

    /// The empty list in RLP format.
    extern vector_ref<std::byte> RLPEmptyList;
