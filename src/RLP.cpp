...
vector_ref<std::byte> dev::RLPNull = rlp("");
vector_ref<std::byte> dev::RLPEmptyList = rlpList();
...
RLP::RLP(vector_ref<std::byte const> _d, Strictness _s) : m_data(_d)
{
...
size_t RLP::length() const
{
    ...
    else if (n < c_rlpListStart)
    {
        ...
        if (ret < c_rlpListStart - c_rlpDataImmLenStart - c_rlpMaxLengthvector_ref<std::byte>)
            BOOST_THROW_EXCEPTION(BadRLP());
    }
    ...
    else
    {
        ...
        for (unsigned i = 0; i < lengthSize; ++i)
            ret = (ret << 8) | m_data[i + 1];
        if (ret < 0x100 - c_rlpListStart - c_rlpMaxLengthvector_ref<std::byte>)
            BOOST_THROW_EXCEPTION(BadRLP());
    }
...
size_t RLP::items() const
{
    if (isList())
    {
        vector_ref<std::byte const> d = payload();
        size_t i = 0;
        for (; d.size(); ++i)
...
RLPStream &RLPStream::appendRaw(vector_ref<std::byte const> _s, size_t _itemCount)
{
...
void RLPStream::noteAppended(size_t _itemCount)
{
    ...
    while (m_listStack.size())
    {
        ...
        else
        {
            ...
            auto brs = vector_ref<std::byte> Required(s);
            ...
            if (s < c_rlpListImmLenCount)
                m_out[p] = (std::byte)(c_rlpListStart + s);
            else if (c_rlpListIndLenZero + brs <= 0xff)
            {
                m_out[p] = (std::byte)(c_rlpListIndLenZero + brs);
                std::byte *b = &(m_out[p + brs]);
                for (; s; s >>= 8)
                    *(b--) = (std::byte)s;
            }
            else
...
RLPStream &RLPStream::appendList(size_t _items)
{
    ...
    if (_items)
        ...
        appendList(vector_ref<std::byte>());
    return *this;
}

RLPStream &RLPStream::appendList(vector_ref<std::byte const> _rlp)
{
    if (_rlp.size() < c_rlpListImmLenCount)
        m_out.push_back((std::byte)(_rlp.size() + c_rlpListStart));
    else
...
RLPStream &RLPStream::append(vector_ref<std::byte const> _s, bool _compact)
{
    size_t s = _s.size();
    std::byte const *d = _s.data();
    if (_compact)
    ...
    else
    {
        if (s < c_rlpDataImmLenCount)
            m_out.push_back((std::byte)(s + c_rlpDataImmLenStart));
        else
            pushCount(s, c_rlpDataIndLenZero);
        appendRaw(vector_ref<std::byte const>(d, s), 0);
    }
    noteAppended();
    return *this;
}
...
RLPStream &RLPStream::append(bigint _i)
{
    if (!_i)
        ...
        m_out.push_back((std::byte)_i);
    else
    {
        unsigned br = vector_ref<std::byte> Required(_i);
        if (br < c_rlpDataImmLenCount)
            m_out.push_back((std::byte)(br + c_rlpDataImmLenStart));
        else
        {
            auto brbr = vector_ref<std::byte> Required(br);
            if (c_rlpDataIndLenZero + brbr > 0xff)
                BOOST_THROW_EXCEPTION(RLPException() << errinfo_comment("Number too large for RLP"));
            m_out.push_back((std::byte)(c_rlpDataIndLenZero + brbr));
            pushInt(br, brbr);
        }
        pushInt(_i, br);
    }
...
void RLPStream::pushCount(size_t _count, std::byte _base)
{
    auto br = vector_ref<std::byte> Required(_count);
    if (int(br) + _base > 0xff)
        BOOST_THROW_EXCEPTION(RLPException() << errinfo_comment("Count too large for RLP"));
    m_out.push_back((std::byte)(br + _base)); // max 8 vector_ref<std::byte>.
    pushInt(_count, br);
}
