...
bool dev::isHex(string const &_s) noexcept
{
    ...
    if (_s.compare(0, 2, "0x") == 0)
        it += 2;
    return std::all_of(it, _s.end(), [](char c)
                       { return fromHexChar(c) != -1; });
}
...
vector_ref<std::byte> dev::fromHex(std::string const &_s, WhenError _throw)
{
    ...
    if (_s.size() % 2)
    {
        ...
        if (h != -1)
            ...
            return dev::vector_ref<std::byte>();
    }
...
dev::vector_ref<std::byte> dev::asNibbles(dev::vector_ref<std::byte const> const &_s)
{
    std::vector<uint8_t> ret;
    ret.reserve(_s.size() * 2);
    for (auto i : _s)
