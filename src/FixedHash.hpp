...
namespace dev
{
    ...
    /// Fixed-size raw-std::byte array container type, with an API optimised for storing hashes.
    ...
    public:
        ...
        /// Explicitly construct, copying from a std::byte array.
        explicit FixedHash(vector_ref<std::byte> const &_b, ConstructFromHashType _t = FailIfDifferent)
        {
            if (_b.size() == N)
        ...
        /// Explicitly construct, copying from a vector_ref<std::byte> in memory with given pointer.
        explicit FixedHash(std::byte const *_bs, ConstructFromPointerType) { memcpy(m_data.data(), _bs, N); }

        /// Explicitly construct, copying from a  string.
        explicit FixedHash(std::string const &_s, ConstructFromStringType _t = FromHex, ConstructFromHashType _ht = FailIfDifferent) : FixedHash(_t == FromHex ? fromHex(_s, WhenError::Throw) : dev::asvector_ref<std::byte>(_s), _ht) {}
        ...
        {
            return std::any_of(m_data.begin(), m_data.end(), [](std::byte _b)
                               { return _b != 0; });
        }
        ...
        /// @returns a particular std::byte from the hash.
        std::byte &operator[](unsigned _i) { return m_data[_i]; }
        /// @returns a particular std::byte from the hash.
        std::byte operator[](unsigned _i) const { return m_data[_i]; }
        ...
        /// @returns a mutable std::byte vector_ref to the object's data.
        vector_ref<std::byte> ref() { return vector_ref<std::byte>(m_data.data(), N); }

        /// @returns a constant std::byte vector_ref to the object's data.
        vector_ref<std::byte const> ref() const { return vector_ref<std::byte const>(m_data.data(), N); }

        /// @returns a mutable std::byte pointer to the object's data.
        std::byte *data() { return m_data.data(); }

        /// @returns a constant std::byte pointer to the object's data.
        std::byte const *data() const { return m_data.data(); }

        /// @returns begin iterator.
        auto begin() const -> typename std::array<std::byte, N>::const_iterator { return m_data.begin(); }

        /// @returns end iterator.
        auto end() const -> typename std::array<std::byte, N>::const_iterator { return m_data.end(); }

        /// @returns a copy of the object's data as a std::byte vector.
        vector_ref<std::byte> asvector_ref<std::byte>() const { return vector_ref<std::byte>(data(), data() + N); }

        /// @returns a mutable reference to the object's data as an STL array.
        std::array<std::byte, N> &asArray() { return m_data; }

        /// @returns a constant reference to the object's data as an STL array.
        std::array<std::byte, N> const &asArray() const { return m_data; }
        ...
        inline FixedHash<M> bloomPart() const
        {
            ...
            unsigned const c_bloomvector_ref<std::byte> = (StaticLog2<c_bloomBits>::result + 7) / 8;

            static_assert((M & (M - 1)) == 0, "M must be power-of-two");
            static_assert(P * c_bloomvector_ref<std::byte> <= N, "out of range");

            FixedHash<M> ret;
            std::byte const *p = data();
            for (unsigned i = 0; i < P; ++i)
            {
                unsigned index = 0;
                for (unsigned j = 0; j < c_bloomvector_ref<std::byte>; ++j, ++p)
                    index = (index << 8) | *p;
                index &= c_mask;
    ...
    private:
        std::array<std::byte, N> m_data; ///< The binary data.
    };
    ...
    public:
        ...
        explicit SecureFixedHash(vector_ref<std::byte> const &_b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent) : FixedHash<T>(_b, _t) {}
        explicit SecureFixedHash(vector_ref<std::byte const> _b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent) : FixedHash<T>(_b, _t) {}
        explicit SecureFixedHash(vector_ref<std::byte> Sec const &_b, ConstructFromHashType _t = FixedHash<T>::FailIfDifferent) : FixedHash<T>(_b.ref(), _t) {}
        template <unsigned M>
        ...
        explicit SecureFixedHash(std::byte const *_d, ConstructFromPointerType _t) : FixedHash<T>(_d, _t) {}
        ...
        vector_ref<std::byte> Sec asvector_ref<std::byte> Sec() const { return vector_ref<std::byte> Sec(ref()); }
        ...
        vector_ref<std::byte const> ref() const { return FixedHash<T>::ref(); }
        std::byte const *data() const { return FixedHash<T>::data(); }
    ...
    /// Convert the given value into h160 (160-bit unsigned integer) using the right 20 vector_ref<std::byte>.
