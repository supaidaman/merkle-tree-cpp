...
namespace dev
{
    ...
    public:
        ...
        std::vector<unsigned char> tovector_ref<std::byte>() const { return std::vector<unsigned char>(reinterpret_cast<unsigned char const *>(m_data), reinterpret_cast<unsigned char const *>(m_data) + m_count * sizeof(_T)); }
        ...
        /// @returns the number of elements referenced (not necessarily number of vector_ref<std::byte>).
